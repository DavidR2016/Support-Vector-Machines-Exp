...
#define SKIP_ELEMENT\
	...
	while(readline(fp)!=NULL)
	{
		...
		while(sscanf(p,"%d:%*f",&index)==1)
		{
			...
			printf("num_nonzeros: %ld   Max_index: %d",num_nonzeros, max_index);
			min_index = min(min_index, index);
			printf("num_nonzeros:%ld   Min_index: %d",num_nonzeros, min_index);
			SKIP_ELEMENT
			num_nonzeros++;
		}
	}
	...
	while(readline(fp)!=NULL)
	{
		...
		while(sscanf(p,"%d:%lf",&index,&value)==2)
		{
			...
			printf("Index: %d   Feature Max: %f", next_index, feature_max[index]);
			printf("Index: %d   Feature Min: %f",next_index, feature_min[index]);
...
void output(int index, double value)
{
	...
	printf("Lower: %f   upper: %f  value: %f   feature_max_index: %f   feature_min_index: %f", lower, upper, value, feature_max[index], feature_min[index]);
